...
/*
This file has the same content as the identifically named md file (`s1_exploratory_analyses.md`), which stores both SQL queries and their outputs for the exploratory analyses. 
...
* Which NOCs performed the best during recent editions?
...
select 
	case when noc_code in ('NPA', 'RPT', 'RPC', 'AIN', 'ROC', 'OAR', 'MAC', 'FRO', 'RUS', 'BLR') then null
...
-- please also see the Notes and Comments at the bottom for additional details


-- the best performing teams at the Paris games
...
-- the best performing teams in each continent
with tmp as (
...
-- how many medals did each continent win?
with tmp as (
select mt2024.*, noc_info.continent,
...
-- So let's aggregate the results of all OGs taking place over the past two Olympic cycles to gain a better picture of their latest sporting strength.
...
-- the best performing NOCs during the past two OG cycles
...
-- best performing teams in each continent during the past two cycles
with tmp as (
...
-- Notes and Comments:
-- you may notice that several NOCs were excluded from ranking in this script
-- this may be due to one of several reasons:
-- NPA, RPC, RPT, MAC and FRO are delegations specific to the paralympics and therefore are not ranked
