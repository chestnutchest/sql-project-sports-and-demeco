...
/*
This file has the same content as the identifically named md file (`s3_population_analysis.md`), which stores both SQL queries and their outputs for the population-related analyses. 
...
-- countries/territories with highest medal per million people at these games
...
-- countries/territories with highest gold medal per million people at these games
select noc_code, noc_name, 
       to_char(populations, '9999999999') as populations, 
...
-- most populous countries/territories that failed to win at least one medal at these games
...
-- most populous countries/territories that failed to win at least one gold medal at these games
select noc_code, noc_name, to_char(populations, '9999999999') as populations, gold, silver, bronze, total
from pa2024_medals_pop
where gold = 0
order by populations desc
...
-- least populous countries/territories that won at least one medal at these games
select noc_code, noc_name, to_char(populations, '9999999999') as populations, gold, silver, bronze, total
from pa2024_medals_pop
where total > 0
...
-- least populous countries/territories that won at least one gold medal at these games
select noc_code, noc_name, to_char(populations, '9999999999') as populations, gold, silver, bronze, total
from pa2024_medals_pop
where gold > 0
order by populations 
...
-- countries/territories with highest gold medals per million population during the past two cycles
select noc_code, noc_name, 
       to_char(populations, '9999999999') as populations, 
       to_char(gold_per_million, '9999.99') as gold_per_million, 
...
-- countries/territories with highest medal per million population during the past two cycles
select noc_code, noc_name, 
       to_char(populations, '9999999999') as populations, 
       to_char(medaL_per_million, '9999.99') as medal_per_million, 
       gold_total, silver_total, bronze_total, 
...
-- most populous countries/territories that failed to win at least one medal over the past two cycles
...
-- most populous countries/territories that failed to win at least one gold medal over the past two cycles
...
-- least populous countries/territories that won at least one medal over the past two cycles
select noc_code, noc_name, to_char(populations, '9999999999') as populations, 
       gold_total, silver_total, bronze_total, 
       total as medal_total
from recent_games_medals_pop
...
-- least populous countries/territories that won at least one gold medal over the past two cycles
select noc_code, noc_name, to_char(populations, '9999999999') as populations, 
       gold_total, silver_total, bronze_total, 
       total as medal_total
from recent_games_medals_pop
